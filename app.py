...
# --- DASHBOARD Y RANKING ---
    ...
    try:
        ...
        if not f_de.empty:
            ...
            f_de['Pte_Firma_E'] = f_de['Estado'].apply(lambda x: 1 if str(x).strip().upper() == "PTE FIRMA" else 0) if 'Estado' in f_de.columns else 0
        ...
        if not f_dt.empty:
            ...
            f_dt['Pte_Firma_F'] = f_dt['Estado'].apply(lambda x: 1 if str(x).strip().upper() == "PTE FIRMA" else 0) if 'Estado' in f_dt.columns else 0
        ...
        if not f_da.empty:
            ...
            f_da['Pte_Firma_A'] = f_da['Estado'].apply(lambda x: 1 if str(x).strip().upper() == "PTE FIRMA" else 0) if 'Estado' in f_da.columns else 0

        r1 = f_de.groupby('Comercial')[['V_Luz', 'V_Gas', 'V_REF', 'Baja_E', 'Cancel_E', 'Pte_Firma_E']].sum() if not f_de.empty else pd.DataFrame()
        r2 = f_dt.groupby('Comercial')[['V_Fibra', 'V_Móvil', 'V_REF', 'Baja_F', 'Cancel_F', 'Pte_Firma_F']].sum() if not f_dt.empty else pd.DataFrame()
        r3 = f_da.groupby('Comercial')[['V_Alarma', 'V_REF', 'Pte_Firma_A']].sum() if not f_da.empty else pd.DataFrame()
        ...
        rank['Pte_Firma_Total'] = rank.filter(like='Pte_Firma_').sum(axis=1)
        rank['Ventas_Sin_Movil'] = (rank.get('V_Luz',0) + rank.get('V_Gas',0) + rank.get('V_Fibra',0) + rank.get('V_Alarma',0))
        
        # Total Neto restando Bajas, Cancelaciones Y Pte Firma
        ...
        # 10. NUEVOS CUADROS: CANCELACIONES, BAJAS Y PTE FIRMA
        st.markdown("<br>", unsafe_allow_html=True)
        ...
        cx4.markdown(f'<div style="{box_alt}"><p style="color:#ff4b4b;font-size:0.75rem;margin:0;">PTE. FIRMA</p><h3 style="color:white;margin:0;">{int(rank["Pte_Firma_Total"].sum())}</h3></div>', unsafe_allow_html=True)
