...
# --- DASHBOARD Y RANKING ---
    ...
    try:
        ...
        with tab_a:
            if not da.empty:
                fig_a = px.bar(da.groupby('Comercial')['V_Alarma'].sum().reset_index().sort_values('V_Alarma'), x='V_Alarma', y='Comercial', orientation='h', text_auto=True, title="Ventas de Alarmas", color_discrete_sequence=['#ff4b4b'])
