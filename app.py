...
# --- FUNCIONES DE DATOS DASHBOARD (ACTUALIZADO PARA NUEVOS NOMBRES) ---
def get_csv_url(url):
    return url.replace('/edit?usp=sharing', '/export?format=csv').split('&ouid=')[0].split('?')[0] + '/export?format=csv'

# URLS ACTUALIZADAS (Asegúrate de que estos enlaces correspondan a los nuevos archivos en Drive)
