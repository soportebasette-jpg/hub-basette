...
st.markdown("""
    ...
    /* Estilos para tarjetas de objetivos */
    .obj-card {
        background-color: #161b22;
...
def load_and_clean_ranking():
    ...
    df_e['Mes_Num'] = df_e['Fecha Creación'].dt.strftime('%m')
    df_e['Mes_Nombre'] = df_e['Fecha Creación'].dt.strftime('%B').upper()
    df_e['Mes'] = df_e['Mes_Num'] + " - " + df_e['Mes_Nombre']
    ...
    df_t['Mes_Num'] = df_t['Fecha Creación'].dt.strftime('%m')
    df_t['Mes_Nombre'] = df_t['Fecha Creación'].dt.strftime('%B').upper()
    df_t['Mes'] = df_t['Mes_Num'] + " - " + df_t['Mes_Nombre']
    ...
    df_a['Mes_Num'] = df_a['Fecha Creación'].dt.strftime('%m')
    df_a['Mes_Nombre'] = df_a['Fecha Creación'].dt.strftime('%B').upper()
    df_a['Mes'] = df_a['Mes_Num'] + " - " + df_a['Mes_Nombre']
    df_a['V_Alarma'] = 1 
    ...
    for idx, item in enumerate(lista_anuncios):
        with cols_anuncios[idx % 3]:
            ...
            if os.path.exists(full_path):
                ...
                st.error(f"No se encontró: {item['file']} en {path_anuncios}")
...
# --- DASHBOARD Y RANKING ---
    ...
    try:
        ...
        tab_r, tab_e, tab_t, tab_a, tab_obj = st.tabs(["🏆 RANKING", "⚡ ENERGÍA", "📱 TELCO", "🛡️ ALARMAS", "🎯 OBJETIVO MENSUAL"])
        ...
        with tab_obj:
            ...
            # Extraer solo el nombre del mes del filtro (ej: "MARCH")
            nombre_mes_filtro = f_mes.split(" - ")[1].upper()
            
            for comercial in f_com:
                # 1. Obtener objetivo del comercial para este mes
                obj_row = df_obj[(df_obj['COMERCIAL'].str.upper() == comercial.upper()) & (df_obj['MES'] == nombre_mes_filtro)]
                
                if not obj_row.empty:
                    objetivo_valor = int(obj_row.iloc[0]['OBJETIVOMES'])
                    
                    # 2. Calcular ventas actuales (Luz + Gas + Fibra + Alarmas) SIN Móviles
                    v_luz = de[de['Comercial'] == comercial]['V_Luz'].sum()
                    v_gas = de[de['Comercial'] == comercial]['V_Gas'].sum()
                    v_fibra = dt[dt['Comercial'] == comercial]['V_Fibra'].sum()
                    v_alarma = da[da['Comercial'] == comercial]['V_Alarma'].sum()
                    
                    actual = int(v_luz + v_gas + v_fibra + v_alarma)
                    restante = int(max(0, objetivo_valor - actual))
                    porcentaje = int((actual / objetivo_valor * 100)) if objetivo_valor > 0 else 0
                    
                    # Visualización
                    with st.expander(f"👤 {comercial.upper()} - {porcentaje}% Completado", expanded=True):
                        c1, c2, c3, c4 = st.columns(4)
                        c1.markdown(f'<div class="obj-card"><div class="obj-title">Ventas Mes</div><div class="obj-val">{actual}</div></div>', unsafe_allow_html=True)
                        c2.markdown(f'<div class="obj-card"><div class="obj-title">Objetivo</div><div class="obj-val">{objetivo_valor}</div></div>', unsafe_allow_html=True)
                        c3.markdown(f'<div class="obj-card"><div class="obj-title">Faltan</div><div class="obj-val">{restante}</div></div>', unsafe_allow_html=True)
                        c4.markdown(f'<div class="obj-card"><div class="obj-title">% Éxito</div><div class="obj-val">{porcentaje}%</div></div>', unsafe_allow_html=True)
                        st.progress(min(porcentaje / 100, 1.0))
                else:
                    st.info(f"No se han encontrado objetivos configurados para {comercial} en el mes: {nombre_mes_filtro}")
