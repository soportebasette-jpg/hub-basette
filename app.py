...
def load_vacaciones():
    try:
        ...
        # Forzar conversión y manejo de errores para evitar el KeyError/ValueError en el calendario
        ...
        cols = ["Comercial", "Fecha Inicio", "Fecha Fin", "Motivo", "Nº DE DIAS"]
        for c in cols:
...
if menu == "🚀 CRM":
    ...
    with t_fibra:
        ...
        # LÍNEAS ADICIONALES DEBAJO DE FIBRA Y MOVIL
        ...
        # LÍNEAS ADICIONALES DEBAJO DE TV
    ...
    # Mostramos las 3 imágenes de la carpeta anunciosbasette en pequeño
    cols_img = st.columns(4) # 4 columnas para que queden pequeñas
    dir_anu = "anunciosbasette"
    if os.path.exists(dir_anu):
        # Filtrar solo archivos de imagen que no sean el QR
        fotos = [f for f in os.listdir(dir_anu) if f.lower().endswith(('.png', '.jpg', '.jpeg')) and 'qr' not in f.lower()]
        for idx, foto in enumerate(fotos[:3]): # Solo las primeras 3
            with cols_img[idx]:
                st.image(os.path.join(dir_anu, foto), use_column_width=True)
    else:
        st.error("Carpeta 'anunciosbasette' no encontrada.")
    ...
    with col_cal:
        ...
        if not df_v.empty:
            # CORRECCIÓN DE ERROR EN CALENDARIO: Asegurar tipos datetime
            ...
            fig = px.bar(df_v, x="Fecha Fin", y="Comercial", base="Fecha Inicio", orientation="h",
                         color="Comercial", title="Cronograma de Ausencias",
                         hover_data=["Motivo", "Nº DE DIAS"])
            fig.update_layout(xaxis_type='date', paper_bgcolor='rgba(0,0,0,0)', 
                             plot_bgcolor='rgba(0,0,0,0)', font_color="white", showlegend=False)
            st.plotly_chart(fig, use_container_width=True)
            st.table(df_v[["Comercial", "Fecha Inicio", "Fecha Fin", "Motivo"]])
        else:
            st.info("No hay datos de vacaciones para mostrar.")
