...
# # --- DASHBOARD Y RANKING ---
    ...
    try:
        ...
        # Marcar Bajas y Cancelaciones por categoría
        if not f_de.empty:
            f_de['Baja_E'] = f_de['Estado'].apply(lambda x: 1 if str(x).strip().upper() == "BAJA" else 0)
            f_de['Cancel_E'] = f_de['Estado'].apply(lambda x: 1 if str(x).strip().upper() == "CANCELADO" else 0)
        if not f_dt.empty:
            f_dt['Baja_F'] = f_dt['Estado'].apply(lambda x: 1 if str(x).strip().upper() == "BAJA" else 0)
            f_dt['Cancel_F'] = f_dt['Estado'].apply(lambda x: 1 if str(x).strip().upper() == "CANCELADO" else 0)

        r1 = f_de.groupby('Comercial')[['V_Luz', 'V_Gas', 'Baja_E', 'Cancel_E']].sum() if not f_de.empty else pd.DataFrame()
        r2 = f_dt.groupby('Comercial')[['V_Fibra', 'V_Móvil', 'Baja_F', 'Cancel_F']].sum() if not f_dt.empty else pd.DataFrame()
        r3 = f_da.groupby('Comercial')[['V_Alarma']].sum() if not f_da.empty else pd.DataFrame()
        
        rank = pd.concat([r1, r2, r3], axis=1).fillna(0)
        
        # --- LÓGICA DEL Nº 1 (SIN MÓVILES) ---
        ...
        # Cálculo Neto para objetivo (restando bajas y cancelaciones)
        rank['Bajas_Totales'] = rank.filter(like='Baja_').sum(axis=1)
        rank['Cancel_Totales'] = rank.filter(like='Cancel_').sum(axis=1)
        rank['Total Neto'] = rank['Ventas_Sin_Movil'] - rank['Bajas_Totales'] - rank['Cancel_Totales']
        ...
        st.markdown(f"""
            <div style="text-align: center; margin: 20px 0;">
                ...
                <div style="background: rgba(210, 255, 0, 0.1); padding: 15px; border-radius: 10px; border: 1px dashed #d2ff00; display: inline-block;">
                    <p style="color: white; margin:0; font-size: 1.3rem;">🥇 Nº 1 DEL MES (Sin Móviles): <b style="color: #d2ff00; text-transform: uppercase;">{lider_real}</b></p>
                </div>
            </div>
        """, unsafe_allow_html=True)

        # 7. TABLA DE RANKING
        df_vis = rank.rename(columns={'V_Luz':'Luz','V_Gas':'Gas','V_Fibra':'Fibra','V_Móvil':'Móvil','V_Alarma':'Alarma','Bajas_Totales':'Bajas','Cancel_Totales':'Cancelados'})
        cols_tab = ['Luz','Gas','Fibra','Móvil','Alarma','Bajas','Cancelados','Total Neto','Faltan para 25']
        ...
        c4.markdown(f'<div style="{box} background:#d2ff00;"><p style="color:black;font-weight:bold;margin:0;">TOTAL BRUTO (S.M.)</p><h2 style="color:black;margin:0;">{int(rank["Ventas_Sin_Movil"].sum())}</h2></div>', unsafe_allow_html=True)

        # 9. CUADROS DE CANCELACIONES Y BAJAS
        st.markdown("<br>", unsafe_allow_html=True)
        cx1, cx2, cx3, cx4 = st.columns(4)
        box_neg = "background:#161b22; border:1px solid #ff4b4b; padding:15px; border-radius:10px; text-align:center;"
        
        cx1.markdown(f'<div style="{box_neg}"><p style="color:#ff4b4b;font-size:0.75rem;margin:0;">CANCEL. ENERGÍA</p><h3 style="color:white;margin:0;">{int(rank["Cancel_E"].sum()) if "Cancel_E" in rank else 0}</h3></div>', unsafe_allow_html=True)
        cx2.markdown(f'<div style="{box_neg}"><p style="color:#ff4b4b;font-size:0.75rem;margin:0;">CANCEL. FIBRA</p><h3 style="color:white;margin:0;">{int(rank["Cancel_F"].sum()) if "Cancel_F" in rank else 0}</h3></div>', unsafe_allow_html=True)
        cx3.markdown(f'<div style="{box_neg}"><p style="color:#ff4b4b;font-size:0.75rem;margin:0;">BAJAS ENERGÍA</p><h3 style="color:white;margin:0;">{int(rank["Baja_E"].sum()) if "Baja_E" in rank else 0}</h3></div>', unsafe_allow_html=True)
        cx4.markdown(f'<div style="{box_neg}"><p style="color:#ff4b4b;font-size:0.75rem;margin:0;">BAJAS FIBRA</p><h3 style="color:white;margin:0;">{int(rank["Baja_F"].sum()) if "Baja_F" in rank else 0}</h3></div>', unsafe_allow_html=True)
