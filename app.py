...
# --- PRECIOS ---
    ...
    with t3:
        ...
        for i, (vel, pre) in enumerate(solo_fibra):
            with f_cols[i]:
                st.markdown(f'<div class="price-card"><b>FIBRA {vel}</b><br><h2 style="color:#d2ff00;">{pre}</h2></div>', unsafe_allow_html=True)

# --- COMPARADOR ---
elif menu == "⚖️ COMPARADOR":
    st.header("Estudio de Ahorro")
    st.write("Sección de comparativa de facturas.")
