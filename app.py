...
if menu == "🚀 CRM":
    ...
    with col_izq:
        ...
        st.link_button("SEGURMA CRM", "https://crm.segurma.com/web#action=619&cids=1&menu_id=200&model=sale.order&view_type=list", use_container_width=True)
    ...
    with col_der:
        ...
        st.link_button("O2 RETAIL", "https://o2online.es/auth/login/?next=%2Fventas%2F&type=retail", use_container_width=True)
        st.link_button("LOWI TOPGESTIÓN", "https://vodafone.topgestion.es/login", use_container_width=True)
...
# --- PRECIOS ---
    ...
    with t3:
        ...
        fibra_movil = [("600 Mb (60GB)", "35€"), ("600 Mb (10+40GB)", "35€"), ("1 Gb (120GB)", "38€")]
        for i, (lin, pre) in enumerate(fibra_movil):
            with fm_cols[i % 3]:
                st.markdown(f'<div class="price-card"><div class="price-title">{lin}</div><div class="price-val">{pre}</div></div>', unsafe_allow_html=True)
...
# --- ANUNCIOS ---
    ...
    lista_anuncios = [
        {"f": "anuncio_alarma1.png", "n": "Alarma Seguridad"},
        {"f": "PUBLI3.jpg", "n": "Energía y Telco"},
        {"f": "600Mb_60Gb y TV.png", "n": "O2 600Mb TV"},
        {"f": "1Gb_30Gb y TV.png", "n": "O2 1Gb TV"},
        {"f": "LINEAS MOVILES ADICIONALES.png", "n": "Líneas Adicionales"}
    ]
    cols = st.columns(3)
    for i, item in enumerate(lista_anuncios):
        with cols[i % 3]:
            full_p = f"{path_anuncios}{item['f']}"
            if os.path.exists(full_p):
                st.image(full_p, caption=item['n'])
                with open(full_p, "rb") as file:
                    st.download_button(f"Descargar {item['n']}", file, file_name=item['f'])
            else:
                st.warning(f"No encontrado: {item['f']}")
...
# --- DASHBOARD ---
    ...
    try:
        ...
        t_r, t_e, t_t, t_a, t_o = st.tabs(["🏆 RANKING", "⚡ ENERGÍA", "📱 TELCO", "🛡️ ALARMAS", "🎯 OBJETIVOS"])
        ...
        with t_o:
            traduccion = {"JANUARY":"ENERO","FEBRUARY":"FEBRERO","MARCH":"MARZO","APRIL":"ABRIL","MAY":"MAYO","JUNE":"JUNIO","JULY":"JULIO","AUGUST":"AGOSTO","SEPTEMBER":"SEPTIEMBRE","OCTOBER":"OCTUBRE","NOVEMBER":"NOVIEMBRE","DECEMBER":"DICIEMBRE"}
            mes_sel = f_mes.split(' - ')[1].upper()
            mes_busqueda = traduccion.get(mes_sel, mes_sel)
            
            ventas_totales = rank[['Comercial', 'TOTAL']].copy()
            obj_mes = df_obj[df_obj['MES'].str.upper() == mes_busqueda]
            
            if not obj_mes.empty:
                comp = pd.merge(ventas_totales, obj_mes, left_on='Comercial', right_on='COMERCIAL', how='left').fillna(0)
                comp['%'] = (comp['TOTAL'] / comp['OBJETIVO'] * 100).replace([float('inf')], 0)
                st.dataframe(comp[['Comercial', 'TOTAL', 'OBJETIVO', '%']])
            else:
                st.info(f"Sin objetivos para {mes_busqueda}")
    ...
    for c in ["GANA ENERGÍA", "NATURGY", "TOTAL", "ENDESA", "O2", "LOWI"]:
        with st.expander(f"📁 DOCUMENTACIÓN {c}"):
            if os.path.exists("manuales"):
                ...
                archivos = [f for f in os.listdir("manuales") if busq in f.lower()]
                for fn in archivos:
