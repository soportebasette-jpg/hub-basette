...
    header { visibility: hidden; }
    ...
    /* Estilo para los labels de los widgets */
...
    
    /* Bloques de cabecera resaltados */
    .block-header {
...
# Se define un contenedor de columnas para alinear título y logo
col_titulo, col_logo_tct = st.columns([3, 1])

with col_titulo:
    st.title("Portales de Gestión")

with col_logo_tct:
    # NUEVO: Inserción del logo tecomparotodo en pequeño a la derecha
    if os.path.exists("tecomparotodo_logo.jpg"):
        st.image("tecomparotodo_logo.jpg", width=150) # Ajusta el ancho según desees

# Se mantiene la cabecera original por si acaso, aunque st.title suele bastar
st.header("Portales de Gestión")

# Resto del código intacto a continuación
...
    path_anuncios = "anunciosbasette/"
    if os.path.exists(path_anuncios):
        # Escaneo dinámico de la carpeta para mostrar TODOS los archivos
...
    st.header("Documentación")
    ...
    # SECCIÓN IBERDROLA MODIFICADA CON EL PDF SOLICITADO
